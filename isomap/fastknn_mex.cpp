...
void fastKNN(double* X, int N, int D, int K, double* ind, double* dis, int verbose) {
    if (verbose)
        mexPrintf(">>>>>>>>>> N=%d, D=%d\n", N, D);
    ...
    for(int n = 0; n < N; n++) {
        ...
        for(int m = 0; m < K; m++) {
			...
			dis[n*K + m] = (double)(distances[m+1]);
        }
    }
		 ...
		 int nrhs, const mxArray *prhs[])
{
	...
	plhs[1] = mxCreateDoubleMatrix(N*K, 1, mxREAL);
    ind = mxGetPr(plhs[0]);
	dis = mxGetPr(plhs[1]);
    ...
    fastKNN(X, N, D, K, ind, dis, verbose);
